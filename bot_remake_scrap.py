...
async def go_to_item_page(post_link, driver):

    # open a link in new tab
    driver.switch_to.new_window('tab')
    driver.get(post_link)
    # time
    posted_time = WebDriverWait(driver, 5).until(EC.presence_of_element_located((By.XPATH, '//span[@class="x193iq5w xeuugli x13faqbe x1vvkbs x1xmvt09 x1lliihq x1s928wv xhkezso x1gmr53x x1cpjm7i x1fgarty x1943h6x x4zkp8e x676frb x1nxh6w3 x1sibtaa xo1l8bm xi81zsa"]'))).text
...
async def scrap_info_front_page(item, driver):
    ...
    # get the time (can get other info, like the description, etc...)
    posted_time = await go_to_item_page(post_link,driver)

    return img_link, post_link, title, price, location, posted_time


# compare data
async def compare_data(img_link, post_link, title, price, location, posted_time):
    # check if the data is already in the csv file

    # read the csv file
    df = pd.read_csv("data.csv",encoding="utf-8")
    post_links_df = df["post_link"].values
    ...
    if post_link not in post_links_df:
        ...
        debug = f'{current_time} New item: {title}, {price}, {location}, {posted_time}'
        print(debug)
        ...
        debug = f'{current_time} Old item: {title}, {price}, {location} {posted_time}'
        print(debug)
...
async def scraping_core(url, channel, driver):
    # get a list of items, can add others like scroll function here...
    ...
    for item in items:
        try:
            ...
            # scrap the info
            img_link, post_link, title, price, location, posted_time = await scrap_info_front_page(item, driver)

            # before sending the info, compare the data to see if there is any new item
            img_link, post_link, title, price, location, posted_time = await compare_data(img_link, post_link, title, price, location, posted_time)
            

            # send the  info to the channel if it is not None
            if title != None:
                await bot_remake_discord.send_embeded(img_link= img_link,post_link= post_link,title= title,price= price,location= location,post_time= posted_time,channel= channel)
